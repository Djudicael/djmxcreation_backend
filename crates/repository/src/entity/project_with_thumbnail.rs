use app_core::dto::{project_dto::ProjectDto, project_with_thumbnail_dto::ProjectWithThumbnailDto};
...
pub struct ProjectWithThumbnail {
    ...
    pub thumbnail_created_on: Option<DateTime<Utc>>,
}
...
impl From<ProjectWithThumbnail> for ProjectWithThumbnailDto {
    fn from(value: ProjectWithThumbnail) -> Self {
        Self::new(
            value.id,
            value.metadata.and_then(value_to_metadata),
            value.visible,
            value.adult,
            Some(value.created_on),
            value.updated_on,
            value.thumbnail_content.and_then(value_to_content),
        )
    }
}
